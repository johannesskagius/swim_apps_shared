...
void main() {
  ...
  group('Invite Tests', () {
    ...
    test('getPendingInvitations: should only return pending invites for the correct club', () async {
      // Arrange: Create a few invites
      final invite1 = CoachInvite(
        clubId: 'club1',
        inviterId: 'c1', inviterName: 'A',
        inviteeEmail: 'a@test.com', sentAt: DateTime.now(),
        status: InviteStatus.pending,
      );
      final invite2 = SwimmerInvite(
        clubId: 'club1',
        inviterId: 'c1', inviterName: 'A',
        inviteeEmail: 'b@test.com', sentAt: DateTime.now(),
        status: InviteStatus.accepted, // Not pending
      );
      final invite3 = CoachInvite(
        clubId: 'club2', // Wrong club
        inviterId: 'c2', inviterName: 'B',
        inviteeEmail: 'c@test.com', sentAt: DateTime.now(),
        status: InviteStatus.pending,
      );

      // Add them all to the 'invites' collection
      await fakeFirestore.collection('invites').add(invite1.toJson());
      await fakeFirestore.collection('invites').add(invite2.toJson());
      await fakeFirestore.collection('invites').add(invite3.toJson());

      // Act
      final pendingInvites = await repository.getPendingInvitations('club1');

      // Assert
      expect(pendingInvites.length, 1);
      expect(pendingInvites.first.inviteeEmail, 'a@test.com');
      expect(pendingInvites.first.role, InviteRole.coach);
    });

    test('deleteInvitation: should delete an invite from the collection', () async {
      // Arrange
      final invite = CoachInvite(
        clubId: 'club1', inviterId: 'c1', inviterName: 'A',
        inviteeEmail: 'a@test.com', sentAt: DateTime.now(),
      );
      final docRef = await fakeFirestore.collection('invites').add(invite.toJson());

      // Act
      await repository.deleteInvitation('club1', docRef.id);

      // Assert
      final doc = await fakeFirestore.collection('invites').doc(docRef.id).get();
      expect(doc.exists, isFalse);
    });
  });
}